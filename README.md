...
- GitHub Actions 自动发布：推 tag 自动在 Windows / Mac 上打包并传到 Release 页面
- 实时匹配结果流：每首歌搜完立刻推给界面，不用等整个歌单跑完才看到
- 自动暂停不再中断整批任务：限流/网络错误/手动点暂停都会正常返回已匹配的部分，可以直接导入
- 手动暂停按钮：点了能在一两百毫秒内真正停下来，不用等当前这首搜完（可中断等待 + 取消飞行中的请求）
- 导入合并到同名歌单，而不是每次都新建，并自动跳过已经在歌单里的曲目
- 修复：切换 tab 会打断正在跑的导出/匹配，现在跑的时候会锁定 tab，后端也加了并发保护防止误触发两个任务
- 修复：网易云"重新登录"按钮点了没反应，还是显示已登录
- 修复：设置页的输入切 tab 后可能丢失（表单状态跟父组件不同步）
- 修复：待机唤醒后请求可能永久卡住不返回（所有请求加超时兜底）
- 修复：Spotify token 刷新失败时报错不清楚，现在会给出具体原因并自动清掉失效的本地登录状态
- 匹配结果列表按最新处理的排在最上面
