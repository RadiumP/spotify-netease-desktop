...
**Windows 上装好之后打不开，提示"智能应用控制已阻止此应用"，怎么办？**

这是 Windows 11（22H2 之后）的"智能应用控制"（Smart App Control，SAC）功能，专门拦截没有代码签名的程序，比 SmartScreen 更严格——没有"仍要运行"这种选项，直接拦死。这个包目前没有代码签名（签名证书一年大几百人民币，个人小工具用不上），所以会被拦。

先试试这个，免费、不影响其他安全防护：
1. 右键安装包 `.exe` → 属性
2. 常规选项卡最下面，如果有"解除锁定"复选框，勾上，点确定
3. 重新运行安装包

这个方法只对部分情况有效。如果还是被拦，就得关掉智能应用控制：**设置 → 隐私和安全性 → Windows 安全中心 → 应用和浏览器控制 → 智能应用控制设置 → 关闭**。

⚠️ **注意：这个操作不可逆**。关掉之后想再打开，必须重置或重装 Windows 才行（微软故意这么设计的，防止被骗着先关后开）。如果这台电脑还有别人用、或者比较在意这层防护，自己权衡一下要不要关。

真正的根治办法是给安装包做代码签名，但成本对这种个人小工具不太划算，暂时没有配。
...
- 传输计时：跑起来的时候显示已用时长
- 限流判断更灵敏：网易云的限流提示消息很明确（"操作频繁，请稍候再试"），检测到就不再重试直接暂停，
  不会像以前一样在明知道被限流的情况下还傻乎乎重试 3-4 次浪费时间
- 自动重试系统：限流/网络问题导致的自动暂停，会在 10-30 分钟（随机）后自动重新开始，不用手动干预；
  只有手动点"暂停"按钮才是真正需要手动重启的暂停。界面上会显示自动重试的倒计时，也可以在等待期间
  随时手动点"从上次继续"提前触发，不用干等
- 断点续传效率优化：之前因为限流/网络问题导致搜索失败的曲目，续传时会自动识别出来重新搜一遍（不是真的
  搜不到，只是当时请求失败了），而"确实在网易云搜不到"的曲目不会重复白搜。这个判断在构建断点数据的时候
  一次性完成，不需要每次续传都重新扫一遍整个歌单
