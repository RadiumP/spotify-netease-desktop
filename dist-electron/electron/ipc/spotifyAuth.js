...
function waitForCallback(state) {
    return new Promise((resolve, reject) => {
        const server = http.createServer((req, res) => {
            const url = new URL(req.url ?? "", types_1.SPOTIFY_REDIRECT_URI);
            ...
            server.close();
            if (error)
                return reject(new Error(`Spotify 拒绝了授权: ${error}`));
            if (!code || returnedState !== state)
                return reject(new Error("state 不匹配，可能是安全问题，重新登录一次"));
            resolve(code);
        });
...
async function loginWithBrowser(clientId) {
    ...
    await electron_1.shell.openExternal(buildAuthUrl(clientId, state, codeChallenge));
